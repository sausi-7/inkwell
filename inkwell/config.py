"""Configuration loader for Inkwell.
...
# Reddit settings
REDDIT_HEADERS = {
    "User-Agent": "Inkwell/0.1 (open source outreach tool)"
}
...
def write_yaml(filename: str, data, directory: Path | None = None) -> Path:
    """Write a dict/list back to a YAML file in the config directory.

    Used by the web UI to save edits to personality.yml / filters.yml /
    subreddits.yml. Writes through CONFIG_DIR by default so the on-disk
    format matches what the CLI reads on scan.
    """
    if directory is None:
        directory = CONFIG_DIR
...
def load_app_prefs(filename: str = "app.yml") -> dict:
    """Load UI-only preferences (output target, last-used settings).

    Kept separate from filters.yml so filter logic stays scoped to filtering.
    """
    prefs = load_yaml(filename)
